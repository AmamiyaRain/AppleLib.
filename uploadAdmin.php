...
function upload ( $file, $upload_dir = './avatar/', $allowSize = 0, $allowType = [] )
{
    ...
    {
        $_SESSION['nowAdmin']['userName'] = $_POST['name'];
        if ($_POST['psd'] == $_POST['psd_C']) {
            $command                       = "UPDATE myDataBase.users SET `name`='{$_SESSION['nowAdmin']['userName']}',`psd`='{$_POST['psd']}' WHERE email='{$_SESSION['nowAdmin']['userEmail']}'";
    ...
    $_SESSION['nowAdmin']['userAvatar'] = $filename;
    $_SESSION['nowAdmin']['userName']   = $_POST['name'];
    if ($_POST['psd'] == $_POST['psd_C']) {
        $command                       = "UPDATE myDataBase.users SET `name`='{$_SESSION['nowAdmin']['userName']}',`psd`='{$_POST['psd']}',`avatar`='{$_SESSION['nowAdmin']['userAvatar']}' WHERE email='{$_SESSION['nowAdmin']['userEmail']}'";
        $_SESSION['tempUploadSuccess'] = true;
...
header ("location:adminNowUser.php");
?>
<!--<img src="--><? //=$a?><!--">-->
</body>
</html>